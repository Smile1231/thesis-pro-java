...
less raw.txt-4| mawk -F ''  '{for (i=1;i<=NF;i++) printf("%s\t", $i);print ""}'|sed 's/\t$//g' > raw.txt-10
...
less ./sgRNA/raw.txt-rank.fa  |grep -v '>'|mawk -F ''  '{for (i=1;i<=NF;i++) printf("%s\t", $i);print ""}'|sed 's/\t$//g' > raw.txt-rank.fa.2
